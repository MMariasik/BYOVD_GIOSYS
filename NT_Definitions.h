...
// ActiveProcessLinks
struct offsets {
    ULONG64 PsInitialSystemProcess; // PEPROCESS PsInitialSystemProcess (czyli wskaźnik na strukturę eprocess)
    ULONG64 Token;
    ULONG64 UniqueProcessId;
    ULONG64 ActiveProcessLinks;
} g_offsets = {
 0xFC6af0,  // PsInitialSystemProcess WIN11 25H2 (ntoskrnl.exe base address - PsInitialSystemProcess address |=| ? nt!PsInitialSystemProcess - nt) 0xfc6af0? kompilacja:26200.8875
 0x248, // Token,
...
int NTGetNtoskrnlInfo(_Out_ PRTL_PROCESS_MODULE_INFORMATION pKernelInfo) {
    if (pKernelInfo == NULL) {
        return 1;
    }
    ...
    if (!ntQuerySystemInformation) return 3;

    ULONG returnLength = 0;

    ntQuerySystemInformation(SystemModuleInformation, NULL, 0, &returnLength);

    PRTL_PROCESS_MODULES modules = (PRTL_PROCESS_MODULES)VirtualAlloc(
        NULL, returnLength, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    ...
    BOOL bFound = 5;
    NTSTATUS status = ntQuerySystemInformation(SystemModuleInformation, modules, returnLength, &returnLength);

    if (status >= 0) {
        for (ULONG i = 0; i < modules->NumberOfModules; i++) {
            PCHAR fullPathName = (PCHAR)modules->Modules[i].FullPathName;
            PCHAR fileName = fullPathName + modules->Modules[i].OffsetToFileName;

            if (_stricmp(fileName, "ntoskrnl.exe") == 0) {
                *pKernelInfo = modules->Modules[i];
                bFound = 0;
                break;
            }
        }
